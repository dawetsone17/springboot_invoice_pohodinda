...
const PersonForm = () => {
    ...
    const handleChange = (e) => {
        ...
        let updatedPerson = { ...person, [name]: value };

        if (name === 'identificationNumber' && value.length === 8) {
            updatedPerson = { ...updatedPerson, taxNumber: `CZ${value}` };
        } else if (name === 'identificationNumber' && value.length !== 8) {
            updatedPerson = { ...updatedPerson, taxNumber: '' };
        }
    ...
    const handleSubmit = async (e) => {
        ...
        ['name', 'identificationNumber', 'taxNumber', 'accountNumber', 'bankCode', 'iban', 'telephone', 'mail', 'street', 'zip', 'city', 'country'].forEach(key => {
            if (key === 'note') return; // Poznámka není povinná
    ...
    return (
        <div className="invoice-form-container">
            ...
            <form onSubmit={handleSubmit} noValidate>
                ...
                <button type="submit">{id ? "Uložit úpravy" : "Vytvořit osobu"}</button>
            </form>
        </div>
    );
};
